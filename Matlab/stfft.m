...
% G contains magnitude (1st row) and frequency (2nd row) computed along the
...
for i=1:length(a)
    ...
    for k=1:n_modes
        w=1/(1-G(1,k));
        filter_r=abs(g0*(1+a(i)))/sqrt(1+a(i)^2+2*a(i)*cos(2*pi*G(2,k)*(1/fs)));
...
% plotting freq response (loop gains at partial frequencies and magnitude
